...
//             Regex::new(r#"var _sessionid = "(?P<sessionid>[A-Z0-9]*)";"#).unwrap();
//     }
//     if let Some(value) = SESSION_ID.captures(web_page_body).unwrap().and_then(|cap| {
//         cap.name("sessionid")
//             .map(|sessionid| sessionid.as_str().to_string())
//     }) {
//         return Ok(value);
//     } else {
//         return Err(());
